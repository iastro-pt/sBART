...
def find_lines_indexes(frame: Frame, skip_orders: Optional[Iterable] = None, include_invalid: bool = False):
    ...
    for order in range(frame.N_orders):
        ...
        derivative_regions = np.where(np.logical_or(np.subtract(first_derivative, 2 * np.asarray(errors)) >= 0,
        ...
        if make_plot:
            ...
            axis[1].scatter(wave[derivative_regions], first_derivative[derivative_regions], color="blue")
