...
class RV_bouchy(RV_routine):
        ...
        for inst in valid_subInst:
            ...
            if self.work_mode == WORKING_MODE.ONE_SHOT:
                ...
                self._output_RVcubes.ingest_cube_into_rolling_skip_reasons(
                    subInst=inst, cube=cube, is_merged=True
                )

            self._output_RVcubes.store_computed_RVs_to_disk(
                dataClassProxy=dataClass,
                which_subInst=inst,
                include_invalid_frames=self._internal_configs[
                    "OUTPUT_WITH_INVALID_FRAMES"
                ],
            )
    ...
    def process_workers_output(
        ...
        for storage_unit in [
            ...
            empty_cube.add_extra_storage_unit(storage_unit)
