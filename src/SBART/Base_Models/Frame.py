...
from SBART.Components import Spectral_Modelling, Spectral_Normalization, Spectrum
...
class Frame(Spectrum, Spectral_Modelling, Spectral_Normalization):
    ...
    **User parameters:**
    ...
    _default_params = DefaultValues(
        ...
        IS_SA_CORRECTED=UserParam(
            ...
            description=(
                ...
                " By default False, as the majority of the instruments do not have it included in their BERV calculation"
            ),
        ),
        REJECT_NEGATIVE_FLUXES=UserParam(
            True,
            constraint=BooleanValue,
            description="Reject any flux value that falls below zero (which can happen due to normalization stages). Default: True",
        ),
    ...
    def add_to_status(self, new_flag: Flag):
    ...
    def store_previous_SBART_result(self, RV: RV_measurement, RV_err: RV_measurement) -> NoReturn:
        ...
        if not isinstance(RV, RV_measurement) or not isinstance(RV_err, RV_measurement):
            raise custom_exceptions.InvalidConfiguration("The previous SBART RVs must be astropy quantities!")
    ...
    def get_header_value(self, kw: str):
        """Directly retrieves a KW from the header. After this is called, the frame will keep the header
        stored in memory until the object is deleted
