...
def apply_approximated_BERV_correction(wave, BERV: float):
...
def apply_BERV_correction(wave, BERV: float):
    # Addint the new term that we found in the ESPRESSO DRS. Source of this is unclear.....
    return wave * (1 + BERV / SPEED_OF_LIGHT)


def remove_BERV_correction(wave, BERV: float):
    return wave / (1 + BERV / SPEED_OF_LIGHT)
