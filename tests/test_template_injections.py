...
def _construct_template(filelist: List[Path], storage_path: Optional[str]) -> Union[SumStellar, Dict[str, Any]]:
    ...
    ModelStell = StellarModel(root_folder_path=Path(storage_path))
...
def test_template_injections():
    ...
    first_template, frame_configs = _construct_template(original_template_list, storage_path=curr_folder / "to_inject")
    ...
    first_template.add_new_frame_to_template(
        frame_to_inject,
    )

    complete_template, _ = _construct_template(full_template_list, storage_path=curr_folder / "complete")

    # TODO: add comparison of the complete and the injected template

    for order in range(frame_to_inject.N_orders):
        wave_C, flux_C, err_C, mask_C = complete_template.get_data_from_spectral_order(
            order=order, include_invalid=False
        )

        wave_i, flux_i, err_i, mask_i = first_template.get_data_from_spectral_order(order=order, include_invalid=False)
